...
export class RecipesService {
  ...
  recipeById(index: number): Recipe {
    //not sure is non-null assertion operator a good practice here, should i secure it by if statements instead?
    //otherwise error: Type 'Recipe | undefined' is not assignable to type 'Recipe'. Type 'undefined' is not assignable to type 'Recipe'.
    return this.recipeList.find(({ id }) => id === index)!;
  }
}
